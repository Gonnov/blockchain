...
class TransactionInput:
    def __init__(self, transaction_output: TransactionOutput, sender_signature :str, sender_address: str, 
...
class Transaction:
    def __init__(self, value: int, receiver_address: str, sender_address: str,sender_signature: str, 
    ...
    def check_transaction_validity(self) -> bool:
        if sys.getsizeof(self) > 4194304: # 4MB
            return False
        if self.transaction_inputs.sender_address == self.transaction_outputs.receiver_address:
            return False
        if self.transaction_inputs.verify_signature('sender_public_key', 'message') is False:
            return False
        if self.transaction_outputs.value <= 0:
            return False
